...
logs = r'C:\Users\SERF1\Desktop\Battery-Safety-Sever-Client\main\Server\Battery-System-database\Battery-System 05-2021'
list_of_files = glob.glob(r'C:\Users\SERF1\Desktop\Battery-Safety-Sever-Client\main\Server\Battery-System-database\Battery-System 05-2021\*') # * means all if need specific format then *.csv
latest_file = max(list_of_files, key=os.path.getctime)
data_file = latest_file
# data_file = r'C:\Users\SERF1\Desktop\Battery-Safety-Sever-Client\main\Server\Battery-System-database\Battery-System 05-2021\log-Battery-System 05-05-2021-10-28-14.csv
## for troubleshooting at home
# data_file = r'/Users/quiana/Documents/UCSD/CER/Data_Processing/Data/Cummins/realtime_plot/combined_logs.csv'

# Create figure for plotting
fig = plt.figure(figsize=(14, 8))
# fig.canvas.manager.window.wm_geometry('+100+0')
ax_main = fig.add_axes([0, 0, 1, 1])
ax_main.axis('off')
im = plt.imread(r'C:\Users\SERF1\PycharmProjects\Dashboard\realtime_plot_background.png')
...
axmodV.set_xlabel('Time [minute]', color='white')
...
def animate(i, xs, ysmodP, ysmodV, ysmodC, origins):
    ...
    max_voltages = [data['BMU01_Max_Cell_Voltage'].iloc[-1], data['BMU02_Max_Cell_Voltage'].iloc[-1],
                    data['BMU03_Max_Cell_Voltage'].iloc[-1], data['BMU04_Max_Cell_Voltage'].iloc[-1],
                    data['BMU05_Max_Cell_Voltage'].iloc[-1], data['BMU06_Max_Cell_Voltage'].iloc[-1],
                    data['BMU07_Max_Cell_Voltage'].iloc[-1], data['BMU08_Max_Cell_Voltage'].iloc[-1]]
    min_voltages = [data['BMU01_Min_Cell_Voltage'].iloc[-1], data['BMU02_Min_Cell_Voltage'].iloc[-1],
                    data['BMU03_Min_Cell_Voltage'].iloc[-1], data['BMU04_Min_Cell_Voltage'].iloc[-1],
                    data['BMU05_Min_Cell_Voltage'].iloc[-1], data['BMU06_Min_Cell_Voltage'].iloc[-1],
                    data['BMU07_Min_Cell_Voltage'].iloc[-1], data['BMU08_Min_Cell_Voltage'].iloc[-1]]
    module_voltages = [data['BMU01_CMA_Voltage'].iloc[-1], data['BMU02_CMA_Voltage'].iloc[-1],
                       data['BMU03_CMA_Voltage'].iloc[-1], data['BMU04_CMA_Voltage'].iloc[-1],
                       data['BMU05_CMA_Voltage'].iloc[-1], data['BMU06_CMA_Voltage'].iloc[-1],
                       data['BMU07_CMA_Voltage'].iloc[-1], data['BMU08_CMA_Voltage'].iloc[-1]]
    temps = [data['BMU01_CMA_Max_Temp'].iloc[-1], data['BMU02_CMA_Max_Temp'].iloc[-1],
             data['BMU03_CMA_Max_Temp'].iloc[-1], data['BMU04_CMA_Max_Temp'].iloc[-1],
             data['BMU05_CMA_Max_Temp'].iloc[-1], data['BMU06_CMA_Max_Temp'].iloc[-1],
             data['BMU07_CMA_Max_Temp'].iloc[-1], data['BMU08_CMA_Max_Temp'].iloc[-1]]

    # Get pump fan status
    temp_warning = data['isPcanTempWarning'].iloc[-1]
    max_temp = max(temps)
    if temp_warning:
    ...
    # xs.append(data['time'].iloc[-1])
    ysmodP.append(data['modbus_Power'].iloc[-1])
    ysmodV.append(data['modbus_Voltage'].iloc[-1])
    ysmodC.append(data['modbus_Current'].iloc[-1])
